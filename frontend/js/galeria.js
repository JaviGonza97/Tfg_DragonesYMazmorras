...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function renderCharacters() {
    ...
    const filtered = allCharacters.filter(
      (c) => (!razaFilter || c.raza === razaFilter) && (!claseFilter || c.clase === claseFilter),
    )
    ...
    document.querySelectorAll(".view-character").forEach((btn) => {
      btn.addEventListener("click", () => {
        const charId = Number.parseInt(btn.dataset.id)
  ...
  function crearFilaEquipo(nombre = "", tipo = "ARMA") {
    ...
    `
    li.querySelector(".remove-equipment-btn").onclick = () => {
      li.remove()
    }
    return li
  }
  ...
  function showDetails(character) {
    ...
    // EDICIÓN DE EQUIPO
    // -------------------
    document.getElementById("edit-equipment-btn").onclick = () => {
  ...
  async function exportPDF(character) {
    ...
    // Usar la nueva función para agregar la imagen
    const imgSize = 45
    await addCharacterImage(doc, 25, currentY + 10, imgSize, character)
